...
export default function Conversation({ navigation }) {
            ...
            const body = {
              ...
              message:
                user.messages.length == 0
                  ? ""
                  : decryptData(user.messages[0].message),
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#191B1B" }}>
      <Spinner
        visible={spinner}
        textContent={"Loading..."}
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity onPress={() => openChat(item)}>
              ...
              <Card style={{ borderRadius: 20, marginTop: 12 }} isDark>
                <CardImage
                  source={{ uri: item.image }}
                  style={{ height: screen.height * 0.2 }}
                />
                <CardTitle
                  title={<Text color="white">{item.username}</Text>}
                  subtitle="The Edhi Foundation provides 24-hour emergency assistance across the entirety of Pakistan and internationally."
